...
# Unblock all PowerShell scripts and modules in the project directory to avoid execution policy warnings.
Get-ChildItem -Path $PSScriptRoot -Recurse -Include *.ps1,*.psm1 | Unblock-File
...
# Ensure the logs directory exists before using $LogPath
...
if (-not $GitHubToken) {
    ...
    if (Test-Path $tokenFilePath) {
        try {
            ...
            if ($tokenObject.GitHubToken -and -not [string]::IsNullOrWhiteSpace($tokenObject.GitHubToken)) {

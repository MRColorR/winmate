...
function Test-ScriptUpdateAvailable {
    ...
    $Repo = $null
    if ($repoPath -match "github\.com/([^/]+)/([^/]+)") { # Matches https://github.com/owner/repo or github.com/owner/repo
        $owner = $matches[1]
        $repositoryName = $matches[2].Replace(".git", "") # Remove .git suffix if present
        $Repo = "$owner/$repositoryName"
    }
    elseif ($repoPath -match "^([^/]+)/([^/]+)$") { # Matches 'owner/repo' format directly
        $Repo = $repoPath
    }
    else {
    ...
    try {
        ...
        try {
            ...
            else { # Current version is same as latest, or newer (e.g., a pre-release or local build).

...
function Get-ManifestDefaultPath {
    ...
    if (-not $PackageId) {
        # Should ideally not be hit due to Mandatory attribute, but good for robustness.
            ...
            if ($output) {
                ...
                foreach ($line in $lines) {
                    if ($line -match '^\s*DefaultInstallLocation:\s*(.+)') {
                        # Regex to find the key and capture its value
...
function Test-AppInstalled {
    ...
    switch ($Provider.ToLower()) {
        ...
        'msstore' {
            ...
            if ($null -ne $wingetMsStoreResult -and $wingetMsStoreResult) {
                # Check if $true explicitly
                $isInstalled = $true
            }
            else {
                ...
                if ($null -ne $wingetResultFallback -and $wingetResultFallback) {
                    # Check if $true explicitly
                    $isInstalled = $true
                }
            }
    ...
    try {
        ...
        switch ($Provider.ToLower()) {
            ...
            'manual' {
                $finalInstallMessage = $installMessageBase
                if ($AppConfig.install_location) {
                    # For manual, install_location is where it *will* be, not a request.
            ...
            'github_release' {
                ...
                elseif ($assetNamePattern -eq 'latest_installer_msi') {
                    ...
                    # Match using -like for wildcard support

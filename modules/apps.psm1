...
.DESCRIPTION
    ...
    This is important for the 'AUTO' install_location feature for Winget packages.
.EXAMPLE
    PS C:\> Test-WingetCreate
    This ensures wingetcreate.exe is available, installing it if necessary.
.NOTES
    This function is a helper, typically called before operations requiring 'wingetcreate' (like Get-ManifestDefaultPath).
    Requires Winget to be installed to function correctly if 'wingetcreate' needs to be installed.
    Logs the outcome of the check or installation attempt.
#>
...
.DESCRIPTION
    ...
    This is useful for determining where an application might be installed if 'install_location: AUTO' is specified in the config.
    Includes retry logic for common issues like GitHub API rate limiting if a GitHub token is not provided or is insufficient.
.PARAMETER PackageId
    The Winget Package ID of the application (e.g., "Microsoft.PowerToys"). This parameter is mandatory.
.PARAMETER GitHubToken
    An optional GitHub Personal Access Token (PAT) to use with 'wingetcreate' for authenticated requests.
    This can help mitigate GitHub API rate limits when 'wingetcreate' fetches manifest data, which often resides on GitHub.
.EXAMPLE
    PS C:\> $defaultPath = Get-ManifestDefaultPath -PackageId "Microsoft.VisualStudioCode" -GitHubToken $myToken
    If successful, $defaultPath will contain the default install location string (e.g., "%ProgramFiles%\Microsoft VS Code")
    as defined in the Visual Studio Code Winget manifest.
.NOTES
    Calls 'Test-WingetCreate' to ensure 'wingetcreate.exe' is available before attempting to use it.
    Returns the 'DefaultInstallLocation' string if found, otherwise returns $null.
    The function implements a retry mechanism with delays specifically for scenarios that might indicate
    GitHub API rate limiting. Logs various stages of its operation for debugging and information.
#>
function Get-ManifestDefaultPath {
    ...
    if (-not $PackageId) { # Should ideally not be hit due to Mandatory attribute, but good for robustness.
        Write-Log "PackageId is required by Get-ManifestDefaultPath." "ERROR"
        return $null
    }
    ...
    for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
        try {
            $tokenArg = if ($GitHubToken) { "--token $GitHubToken" } else { "" } # Use token if provided
            # Execute wingetcreate to show the installer manifest content. Errors are suppressed for custom handling.
            ...
            if ($output) {
                ...
                $lines = $output -split "`r`n|`n|`r" # Handles CRLF, LF, CR newlines
                foreach ($line in $lines) {
                    if ($line -match '^\s*DefaultInstallLocation:\s*(.+)') { # Regex to find the key and capture its value
                        $foundPath = $Matches[1].Trim()
                        Write-Log "Found DefaultInstallLocation in manifest for '$PackageId': '$foundPath'" "DEBUG"
                        return $foundPath # Return the found path
                    }
                }
                # If loop completes and DefaultInstallLocation wasn't found in a (seemingly valid) output.
                Write-Log "DefaultInstallLocation not found in the manifest output for '$PackageId'. The manifest might not define it." "DEBUG"
                return $null # Explicitly return null if not found after parsing.
            } else {
                 # No output from wingetcreate, could be invalid PackageId or no manifest.
                 Write-Log "No output from 'wingetcreate show' for '$PackageId' (Attempt $attempt/$maxRetries). PackageId might be invalid or manifest unavailable." "DEBUG"
                 return $null # No manifest data to parse.
            }
        }
        catch {
            # Catch errors from wingetcreate execution itself (e.g., tool crashes, unexpected errors).
            Write-Log "Error executing 'wingetcreate show' for '$PackageId' (Attempt $attempt/$maxRetries): $($_.Exception.Message)" "WARNING"
            # Depending on the error, a retry might be useful, but often such errors are persistent.
            break # Exit loop on error from wingetcreate itself.
        }
    }
    Write-Log "Failed to retrieve DefaultInstallLocation for '$PackageId' after $maxRetries attempts or due to an error." "INFO"
    return $null
}

<#
.SYNOPSIS
    Determines the default installation location for an application, trying manifest first then system defaults.
.DESCRIPTION
    This function attempts to find an appropriate default installation location for an application.
    1. If a 'PackageId' is provided (typically for Winget packages), it first calls 'Get-ManifestDefaultPath'
       to try and retrieve the 'DefaultInstallLocation' specified in the package's manifest. If found, this path
       (with environment variables expanded) is returned.
    2. If the manifest does not provide a location, or if no 'PackageId' was given, the function falls back to
       common system default locations. It checks in order: 'Program Files', then 'Program Files (x86)'.
    3. As a last resort, if neither Program Files path is suitable (e.g., on some systems), it defaults to an 'Apps'
       folder in the root of the primary system drive (e.g., C:\Apps), creating this folder if it doesn't exist.
    If an 'AppName' is also provided, this name is appended as a subdirectory to the chosen base path to form the final path.
.PARAMETER PackageId
    Optional. The Winget Package ID of the application (e.g., "Microsoft.PowerToys"). If provided, the function
    will attempt to get the default location from the package manifest first.
.PARAMETER GitHubToken
    Optional. A GitHub Personal Access Token (PAT) passed to 'Get-ManifestDefaultPath' if 'PackageId' is specified,
    to help avoid GitHub API rate limits.
.PARAMETER AppName
    Optional. The name of the application. If provided, this will be appended as a subdirectory to the
    determined base path (e.g., if base is "C:\Program Files", result becomes "C:\Program Files\AppName").
.EXAMPLE
    PS C:\> Get-DefaultInstallLocation -PackageId "Microsoft.PowerToys" -AppName "PowerToys"
    This first tries to get the default install location from the PowerToys Winget manifest. If not found,
    it determines a system default (like "C:\Program Files") and appends "PowerToys" to it.

.EXAMPLE
    PS C:\> Get-DefaultInstallLocation -AppName "MyCustomApp"
    Since no PackageId is given, this determines a system default path (e.g., "C:\Program Files")
    and appends "MyCustomApp", returning something like "C:\Program Files\MyCustomApp".
.NOTES
    Returns a string representing the determined default installation path.
    Environment variables found in paths retrieved from Winget manifests (e.g., "%PROGRAMFILES%") are automatically expanded.
...
function Get-DefaultInstallLocation {
    ...
    if ($PackageId) {
        ...
        if ($manifestPath) {
            ...
            Write-Log "Could not retrieve DefaultInstallLocation from manifest for '$PackageId'. Will use system default logic." "INFO"
        }
    }

    # Fallback to system default install locations if manifest path isn't found or PackageId isn't provided.
    Write-Log "Determining system default install location for '$AppName' (or PackageId '$PackageId')." "INFO"
    $programFilesPath = [Environment]::GetFolderPath('ProgramFiles')
    $programFilesX86Path = [Environment]::GetFolderPath('ProgramFilesX86')
    $chosenBasePath = $null

    # Prefer standard Program Files directories.
    if (Test-Path $programFilesPath) {
        ...
        # Last resort: Use a generic 'Apps' folder on the system drive if Program Files paths are not found (unusual).
        $systemDrive = ($psdrive | Where-Object { $_.Provider.Name -eq 'FileSystem' -and $_.Root -match '^[A-Za-z]:\\$' } | Select-Object -First 1).Root
        if (-not $systemDrive) { $systemDrive = $env:SystemDrive + "\" } # Fallback if PSDrive method fails to find a suitable drive.
        $chosenBasePath = Join-Path $systemDrive 'Apps' # Default to C:\Apps or similar.
        # Ensure this fallback directory exists.
        if (-not (Test-Path $chosenBasePath)) {
            try {
                Write-Log "Attempting to create fallback installation directory: '$chosenBasePath'" "DEBUG"
                New-Item -Path $chosenBasePath -ItemType Directory -Force -ErrorAction Stop | Out-Null
            } catch {
                 Write-Log "Failed to create fallback directory '$chosenBasePath'. Error: $($_.Exception.Message). Defaulting to user TEMP." "WARNING"
                 # If directory creation fails (e.g. permissions), default to something like user's TEMP directory as an absolute last resort.
    ...
    if ($AppName) {
        ...
        # Return just the base path if no AppName is specified for a subdirectory.
...
.DESCRIPTION
    ...
    - For 'msstore': Delegates the check to the 'winget' provider logic, attempting with '--source msstore' first, then fallback.
.PARAMETER AppName
    The display name or key of the application (used for logging and as a fallback ID). Mandatory.
.PARAMETER AppConfig
    The application's configuration object, containing at least 'package_id' (or uses AppName as ID)
    and optionally 'install_location' for manual checks. Mandatory.
.PARAMETER Provider
...
.NOTES
    ...
    Logs its actions and findings. For 'manual'/'github_release', the accuracy depends on 'install_location'
    being correctly specified in the config and accurately reflecting the actual installation state (e.g., pointing to a key executable or main folder).
    The $appIdToCheck defaults to $AppName if $AppConfig.package_id is not present.
#>
function Test-AppInstalled {
    ...
    $isInstalled = $false
    # Use package_id from config if available, otherwise default to the AppName (key from config).
    $appIdToCheck = if (-not [string]::IsNullOrEmpty($AppConfig.package_id)) { $AppConfig.package_id } else { $AppName }

    switch ($Provider.ToLower()) {
        'winget' {
            # Winget list --id <id> is reliable for checking.
            $wingetOutput = winget list --id $appIdToCheck --accept-source-agreements 2>$null
            # Check exit code AND if the output string contains the ID (winget can sometimes return 0 for "no applicable results").
            if ($LASTEXITCODE -eq 0 -and $wingetOutput -match [regex]::Escape($appIdToCheck)) {
                $isInstalled = $true
            }
        }
        'chocolatey' {
            ...
            if ($LASTEXITCODE -eq 0 -and $chocoListOutput -match [regex]::Escape($appIdToCheck)) {
                $isInstalled = $true
            }
        }
        'scoop' {
            ...
            # Check for non-error exit code and output that doesn't explicitly state "not installed" or similar.
            if ($LASTEXITCODE -eq 0 -and $scoopStatusOutput -notmatch "(?i)error|not installed|missing|Couldn't find") {
                 # Further check if the output actually confirms the app by name or ID.
                 # Scoop's 'status' output for an installed app usually includes a line like "Name: <appIdToCheck>"
                 # or lists the version directly next to the app name.
                 if ($scoopStatusOutput -match "Name:\s*$(Invoke-Expression "[regex]::Escape($appIdToCheck)")" -or $scoopStatusOutput -match "^\s*$(Invoke-Expression "[regex]::Escape($appIdToCheck)")\s+\S+") {
                     $isInstalled = $true
                 }
            }
        }
        'manual' {
            # For manual or GitHub release installations, rely on 'install_location' in config.
            # This path should point to a key file or directory that indicates installation.
            if ($AppConfig.install_location) {
                ...
                if (Test-Path $installPath) {
                    ...
                    # $isInstalled remains false or its previous value (which is $false initially)
                }
            } else {
                ...
                # $isInstalled remains false as we cannot verify.
            }
        }
        'github_release' {
            # Logic for github_release check (currently same as manual, using AppConfig.install_location)
            if ($AppConfig.install_location) {
                $installPath = [Environment]::ExpandEnvironmentVariables($AppConfig.install_location)
                if (Test-Path $installPath) {
                    Write-Log "App '$AppName' (Provider: $Provider) presumed installed based on presence of install_location: '$installPath'." "INFO"
        ...
        'msstore' {
            Write-Log "App '$AppName' (provider $Provider): Checking via Winget." "DEBUG"
            # Attempt to check via winget --source msstore first
            $packageIdForWingetMsStore = if ($AppConfig.package_id) { $AppConfig.package_id } else { $AppName }
            $wingetMsStoreResult = winget list --id $packageIdForWingetMsStore --source msstore --accept-source-agreements 2>$null | Select-String -Pattern $packageIdForWingetMsStore -Quiet

            if ($null -ne $wingetMsStoreResult -and $wingetMsStoreResult) { # Check if $true explicitly
                $isInstalled = $true
            } else {
                # Fallback to checking default winget source if not found in msstore source, as some store apps might list under default winget
                Write-Log "App '$AppName' not confirmed via winget --source msstore. Checking default winget source." "DEBUG"
                $packageIdForWingetDefault = if ($AppConfig.package_id) { $AppConfig.package_id } else { $AppName } # Re-evaluate ID if needed, though likely same
                $wingetResultFallback = winget list --id $packageIdForWingetDefault --accept-source-agreements 2>$null | Select-String -Pattern $packageIdForWingetDefault -Quiet
                if ($null -ne $wingetResultFallback -and $wingetResultFallback) { # Check if $true explicitly
                    $isInstalled = $true
                }
            }
        }
    ...
    if ($isInstalled) {
        ...
        Write-Log "App '$AppName' (Provider: $Provider, ID: '$appIdToCheck') not found or not confirmed installed by checks." "INFO"
    }
    return $isInstalled
}
...
function Install-SingleApp {
    ...
    $installMessageBase = "Installing '$AppName'"
    ...
    try {
        ...
        switch ($Provider.ToLower()) {
            'winget' {
                ...
                Invoke-Expression $wingetCmd # Execute the command
            ...
            'msstore' {
                ...
                Invoke-Expression $msStoreCmd # Execute the command
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Invoke-Expression $wingetFallbackCmd # Execute fallback command
                    if ($LASTEXITCODE -eq 0) {
